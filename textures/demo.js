...
export function createDemo(divId) {
    ...
    function initUI() {
        ...
        const updateWeightPerturbation = () => {
            const val = parseFloat($('#weightPerturbation').value);
            $('#weightPerturbationLabel').textContent = val.toFixed(2);
            params.weightPerturbation = val;
            if (ca) {
                ca.updatePerturbationMagnitude(val);
            }
        };
        $('#weightPerturbation').onchange = updateWeightPerturbation;
        $('#weightPerturbation').oninput = updateWeightPerturbation;

        // Reset perturbation button
        $('#resetPerturbation').onclick = () => {
            if (ca) {
                ca.resetPerturbations();
            }
        };

        // Pruning fraction slider
        const updatePruningFraction = () => {
            const val = parseFloat($('#pruningFraction').value);
            $('#pruningFractionLabel').textContent = val.toFixed(2);
            params.weightPruningFraction = val;
            if (ca) {
                ca.updatePruningFraction(val);
            }
        };
        $('#pruningFraction').onchange = updatePruningFraction;
        $('#pruningFraction').oninput = updatePruningFraction;

        // Reset pruning button
        $('#resetPruning').onclick = () => {
            if (ca) {
                ca.resetPruning();
            }
        };
