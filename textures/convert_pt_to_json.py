...
def detect_model_type(state_dict, chn=12):
    ...
    if input_dim == 2 * chn:
        print(f"Detected RD model: input_dim={input_dim} = 2 × {chn} (identity + laplacian)")
        return 'rd'
    ...
    for i, layer in enumerate(np_models[:-1]):
        ...
        if i == 0:
            ...
            if metadata.get('model_type') == 'rd':
                # RD model: 2 filters (identity + laplacian)
                chn = filter_channels // 2
                if filter_channels % 2 != 0:
                    raise ValueError(f"Expected RD 2-filter model but filter_channels ({filter_channels}) is not divisible by 2")

                # For RD, filters are already in correct order: [identity channels, laplacian channels, bias]
                # No rearrangement needed
                print(f"RD model layer 0: {chn} channels × 2 filters = {filter_channels} filter inputs")
